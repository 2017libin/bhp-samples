...
def handler(chan, host, port):
    ...
    while True:
        ...
        r, w, x = select.select([sock, chan], [], [])  # 不断监控sock或者chan是否发生变换，将发生变化的返回到r中
        if sock in r:
            data = sock.recv(1024)
            if len(data) == 0:
                break
...
def reverse_forward_tunnel(server_port, remote_host, remote_port, transport):
    transport.request_port_forward("", server_port)  # 接收数据的端口
    while True:
        chan = transport.accept(1000)
        if chan is None:
            continue
        thr = threading.Thread(
            target=handler, args=(chan, remote_host, remote_port)  # 将server_port中的数据转发到(remote_host:remote_port)
        )
        thr.setDaemon(True)
        thr.start()
